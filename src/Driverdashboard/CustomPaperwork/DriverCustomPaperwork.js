...
console.log(userId)
    if (userId) {
      axios
        .get(`https://isovia.ca/fms_api/api/tipsdriverfetchProductData/${userId}`)
...
console.log(formData);
  ...
  return (
    <div  className="content-wrapper" style={{ minHeight: 440 }}>
     ...
     <tbody>
  {paperworkList.filter(doc => doc.user_id == userId).length === 0 ? (
    ...
    <tr key={doc.id}>
      <td>
        {tripList.find((d) => d.id === doc.trip_id)
          ? `${tripList.find((d) => d.id === doc.trip_id).customer_orderno} `
          : "Unknown Trip"}
      </td>
      <td>
