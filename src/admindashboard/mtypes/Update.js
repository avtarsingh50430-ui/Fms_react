...
const Updatemtype = () => {
  ...
  useEffect(() => {
    const fetchTrailerData = async () => {
      try {
        const response = await axios.get(`https://isovia.ca/fms_api/api/updatemtypes/${id}`);
        setFormData(response.data.product_data);
      } catch (error) {
        console.error('Error fetching mtypes data:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://isovia.ca/fms_api/api/updatemtypes/${id}`, formDataToSend, {
        headers: {
      ...
      alert('mtypes updated successfully!');
    } catch (error) {
      console.error('Error updating mtypes data:', error);
      alert('Error updating mtypes data.');
    }
  };
  return (
