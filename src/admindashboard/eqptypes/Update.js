...
const Updateeqptypes = () => {
  ...
  useEffect(() => {
    const fetchTrailerData = async () => {
      try {
        const response = await axios.get(`https://isovia.ca/fms_api/api/updateeqptypes/${id}`);
        setFormData(response.data.product_data);
      } catch (error) {
        console.error('Error fetching eqptypes data:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://isovia.ca/fms_api/api/updateeqptypes/${id}`, formDataToSend, {
        headers: {
      ...
      alert('eqptypes updated successfully!');
    } catch (error) {
      console.error('Error updating eqptypes data:', error);
      alert('Error updating eqptypes data.');
    }
  };
  return (
