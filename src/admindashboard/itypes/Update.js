...
const Updateitypes = () => {
  ...
  useEffect(() => {
    const fetchTrailerData = async () => {
      try {
        const response = await axios.get(`https://isovia.ca/fms_api/api/updateitypes/${id}`);
        setFormData(response.data.product_data);
      } catch (error) {
        console.error('Error fetching itypes data:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://isovia.ca/fms_api/api/updateitypes/${id}`, formDataToSend, {
        headers: {
      ...
      alert('itypes updated successfully!');
    } catch (error) {
      console.error('Error updating itypes data:', error);
      alert('Error updating itypes data.');
    }
  };
