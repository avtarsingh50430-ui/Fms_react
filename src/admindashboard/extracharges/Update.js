...
const Updateextracharges = () => {
  ...
  useEffect(() => {
    const fetchTrailerData = async () => {
      try {
        const response = await axios.get(`https://isovia.ca/fms_api/api/updateextracharges/${id}`);
        setFormData(response.data.product_data);
      } catch (error) {
        console.error('Error fetching extracharges data:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://isovia.ca/fms_api/api/updateextracharges/${id}`, formDataToSend, {
        headers: {
      ...
      alert('extracharges updated successfully!');
    } catch (error) {
      console.error('Error updating extracharges data:', error);
      alert('Error updating extracharges data.');
    }
  };
  return (
            ...
            <div className="box-body">
              <div className="col-md-6 col-xs-12 pull pull-left">
                ...
                <div className="col-md-3 col-xs-12 pull pull-left">
                  <div className="form-group">
                    <label htmlFor="store">Charge Type</label>
                    <select
                      className="form-control"
                      id="chargetype"
                      name="chargetype"
