...
const Updatefleets = () => {
  ...
  useEffect(() => {
    const fetchTrailerData = async () => {
      try {
        const response = await axios.get(`https://isovia.ca/fms_api/api/updatefleet/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://isovia.ca/fms_api/api/updatefleet/${id}`, formDataToSend, {
        headers: {
      ...
      alert('updatefleet updated successfully!');
    } catch (error) {
      console.error('Error updating updatefleet data:', error);
      alert('Error updating updatefleet data.');
    }
  };
  return (
